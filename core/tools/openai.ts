...
export async function openAiCompatibleChat(params: OpenAICompatibleChatParams): Promise<{ content: string; think?: string }> {
  ...
  const { content: finalContent, think } = splitThinkContent(content)
  return { content: finalContent, think: think || undefined }
}
...
export async function openAiCompatibleChatWithTools(params: {
  url: string
  apiKey?: string
  model: string
  messages: OpenAiMessage[]
  ...
  signal?: AbortSignal
}): Promise<{ content: string; think?: string; toolCalls: OpenAiToolCall[] }> {
  const headers: Record<string, string> = {
    'Content-Type': 'application/json',
  }
  if (params.apiKey) headers.Authorization = `Bearer ${params.apiKey}`
  ...
  const json: any = await res.json()
  const message: any = json?.choices?.[0]?.message || {}
  ...
  const toolCalls: OpenAiToolCall[] = Array.isArray(message?.tool_calls)
    ? message.tool_calls
        ...
        .filter((call: OpenAiToolCall) => call.name)
  ...
  return { content, think: think || undefined, toolCalls }
}
